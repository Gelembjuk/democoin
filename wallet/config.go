...
func GetAppInput() (wallet.AppInput, error) {
	...
	datadirPtr := cmd.String("datadir", "", "Location of data files, config")
...
func checkNeedsHelp(c wallet.AppInput) bool {
	if c.Command == "help" || c.Command == "" {
		return true
	}
	return false
}
func checkConfigUpdateNeeded(c wallet.AppInput) bool {
...
func updateConfig(datadir string, c wallet.AppInput) error {
	config := wallet.AppInput{}
...
func printUsage() {
	...
	fmt.Println("  showunspent -address ADDRESS - Displays the list of all unspent transactions and total balance")
	fmt.Println("  showhistory -address ADDRESS - Displays the wallet history. All In?Out transactions")
	fmt.Println("  getbalance -address ADDRESS - Get balance of ADDRESS")
	fmt.Println("  listaddresses - Lists all addresses from the wallet file")
	fmt.Println("  listbalances - Lists all addresses from the wallet file and show balance for each")
	fmt.Println("  send -from FROM -to TO -amount AMOUNT - Send AMOUNT of coins from FROM address to TO. ")
	fmt.Println("  setnode -nodehost HOST -nodeport PORT - Saves a node host and port to configfile. ")
}
